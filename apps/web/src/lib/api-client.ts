...
export async function generatePack(request: GeneratePackRequest) {
  return apiFetch<PackManifest>("/generate", {
    method: "POST",
...
export async function getPacks() {
  return apiFetch<PackSummary[]>("/packs");
}

export async function getPack(packId: string) {
  return apiFetch<PackManifest>(`/packs/${packId}`);
}

export async function getPackStats() {
  return apiFetch<PackStats>("/packs/stats");
}

export async function getPackActivity(days = 7) {
  return apiFetch<DailyPackCount[]>(`/packs/stats/activity?days=${days}`);
}

export async function getStickerUrl(packId: string, key: string) {
  return apiFetch<{ url: string }>(`/packs/${packId}/sticker-url`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function exportPack(packId: string, platform: Platform) {
  return apiFetch<ExportResult>(`/packs/${packId}/export`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function deletePack(packId: string) {
  return apiFetch<{ deleted: boolean; pack_id: string; objects: number }>(
    `/packs/${packId}`,
    { method: "DELETE" }
  );
}
