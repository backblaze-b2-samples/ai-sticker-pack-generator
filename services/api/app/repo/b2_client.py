...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-ai-sticker-pack-generator",
        ),
    )
...
def put_bytes(key: str, data: bytes, content_type: str) -> None:
    """Raw S3 put_object for in-memory artifacts (manifests, export ZIPs).

    Thinner than upload_file (no FileMetadata round-trip). Raises
...
def get_object_bytes(key: str) -> bytes | None:
    """Read an object's bytes from B2 via S3 get_object.

    Returns None if the object does not exist; raises RuntimeError on any
    other S3 failure. Used to read stickers back when assembling export
    ZIPs and to read pack manifests.
    """
    client = get_s3_client()
    try:
...
def delete_prefix(prefix: str) -> int:
    """Delete every object under a prefix via batched S3 delete_objects.

    Paginates the listing and deletes in batches of up to 1000 keys (the
    S3 delete_objects limit). Returns the number of objects deleted.
