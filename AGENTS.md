...
## 2. App Surface (forked from vibe-coding-starter-kit)

This app forks the starter kit. The starter contract still holds: keep the reusable B2-backed scaffolding, adapt the dashboard, and build new screens with the existing primitives.
...
- **File Explorer.** `/files` route, `apps/web/src/app/files/`, and `apps/web/src/components/files/` (full-bucket browse). The Files sidebar entry in `apps/web/src/components/layout/app-sidebar.tsx` stays.
- **Upload.** `/upload` route, `apps/web/src/app/upload/`, and `apps/web/src/components/upload/` (also the way to drop in your own reference images). The Upload sidebar entry stays.

**Sticker-specific surface (this app's primary feature)**
- **Generate.** `/generate` route + `apps/web/src/components/generate/`. Calls `POST /generate`. See [docs/features/sticker-generation.md](docs/features/sticker-generation.md).
- **Packs library.** `/packs` + `/packs/[id]` routes + `apps/web/src/components/packs/`. Scoped to the `sticker-packs/` B2 prefix — the sample-specific counterpart to the full-bucket `/files` explorer. See [docs/features/sticker-packs.md](docs/features/sticker-packs.md) and [docs/features/pack-export.md](docs/features/pack-export.md).
- Backend: `runtime/stickers.py`, `runtime/packs.py`; `service/stickers.py`, `service/packs.py`; `repo/image_client.py` (OpenAI adapter — SDK imported lazily inside the function, never at module scope or outside `repo/`); `types/stickers.py`.

**Adapt to the use case**
- **Dashboard.** `/` route and `apps/web/src/components/dashboard/` show sticker metrics (total packs, total stickers, generated this week, pack storage), a packs-per-day chart, and a recent-packs table. New aggregations flow through the same `runtime -> service -> repo` layering and are exposed via TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.
- Update `docs/features/dashboard.md` in the same PR as any dashboard change (see §9).

**Why this contract exists**
- The UI kit, Files, and Upload pages are the reusable B2-backed scaffolding. The dashboard and the sticker surface (Generate + Packs) are the app-specific screens.
