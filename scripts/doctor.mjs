...
// OpenAI key required for sticker generation. Surfaced as a fatal check
// alongside the B2 vars so the gotcha is caught before `pnpm dev`.
...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-ai-sticker-pack-generator)",
    );
  }

  // OpenAI key — sticker generation can't work without it.
  const openaiMissing = REQUIRED_OPENAI_VARS.filter(
    (k) => !env[k] || PLACEHOLDERS.has(env[k]),
  );
  if (openaiMissing.length > 0) {
    fail(
      `.env is missing or has placeholder OpenAI variables: ${openaiMissing.join(", ")}`,
      "Sticker generation needs an OpenAI API key. Add OPENAI_API_KEY to .env (note: gpt-image-1 may require OpenAI organization verification).",
    );
  }
}
